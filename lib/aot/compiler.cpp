...
Expect<void> outputWasmLibrary(const std::filesystem::path &OutputPath,
  ...
  llvm::SmallString<0> OSCustomSecVec;
  {
    ...
    for (auto &Section : ObjFile->sections()) {
      ...
      if (auto Name = Section.getName()) {
        spdlog::debug("name:{} address:0x{:x} size:0x{:x} {} {} {}", *Name,
                      Address, Size, Section.isText(), Section.isData(),
                      Section.isBSS());
      } else {
        spdlog::debug("address:0x{:x} size:0x{:x} {} {} {}", Address, Size,
